...
# 스크래핑 및 전처리 실행
schedule.every().day.at("01:00").do(udbstock)

# 주가 예측 모델 실행
schedule.every().day.at("01:10").do(predic.pred_machine(stock_name_list))

# 포트폴리오 모델 실행
# schedule.every().day.at("06:20").do(opti.merge_predictions(stock_name_list))
schedule.every().day.at("08:25").do(opti.pf1(merged_data))
schedule.every().day.at("08:26").do(opti.pf2(merged_data))
schedule.every().day.at("08:27").do(opti.pf3(merged_data))
schedule.every().day.at("08:28").do(opti.pf4(merged_data))
schedule.every().day.at("08:29").do(opti.pf5(merged_data))
