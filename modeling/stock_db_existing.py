...
def loadData(stockname):  # 엑셀데이터 db에 저장
    try:
        file = pd.read_excel('C:/Users/server03_2/Desktop/stock/{}.xlsx'.format(stockname))
    except FileNotFoundError as ex:
        print(ex)
    else:
        file.to_sql('{}'.format(stockname), conn, if_exists='replace', index=False)  # 기업마다 다른 테이블로 저장
